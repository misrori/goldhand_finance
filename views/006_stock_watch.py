...
def update_index(filtered_df):
    st.session_state.selected_filtered_index = int(filtered_df.loc[filtered_df['display_name'] == st.session_state.selected_filtered_id].index[0])


# st index
if 'stw_selected_indexes' not in st.session_state:
    st.session_state.stw_selected_indexes = []
...
def show_stock_filter():
    ...
    with tab1:  
        ...
        st.session_state.stw_selected_indexes = filtered_df['ticker'].tolist()
    ...
    with tab2:
        ...
        temp_df = tw.stock.loc[tw.stock["name"].isin(st.session_state.stw_selected_indexes)]
        ...
        with col1:
            label_of_next_button = f'Next: {temp_df["display_name"].tolist()[(st.session_state.selected_filtered_index + 1) % len(temp_df)]}'
